...
def output_status(
    ...
    if time_to_now:
        logging.info(
            f"{timestamp} - Podpings: {pings:7,} / {total_pings:10,} - Count: {count_posts} "
            f"- BlockNum: {current_block_num} - Time Delta: {time_to_now}"
        )
    else:
        logging.info(
            f"{timestamp} - Podpings: {pings:7,} / {total_pings:10,} - Count: {count_posts} - "
            f"BlockNum: {current_block_num}"
        )


def output_to_socket(post, client_socket) -> None:
    ...
    for post in stream:
        ...
        if reports:
            if time_dif > report_freq:
                ...
                output_status(
                    timestamp, pings, count_posts, current_block_num=current_block_num
                )
...
def scan_history(param=None, report_freq=5, reports=True):
    ...
    if post:
        ...
        logging.info(
            "Finished catching up at block_num: "
            + str(post["block_num"])
            + " in "
            + str(scan_time)
        )
